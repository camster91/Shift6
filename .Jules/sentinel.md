...
## 2026-06-10 - Production Error Boundaries and Information Disclosure Mitigation
**Vulnerability:** Information leakage of internal application state, call stacks, and sensitive runtime parameters via unsuppressed error boundaries in production.
**Learning:** Displaying raw error objects and component stacks to end-users exposes database column names, API structures, paths, and local state. Developers need these during local testing, but they must be completely omitted in production to thwart potential attackers seeking to exploit application internals.
**Prevention:** Wrap all UI rendering of error details, raw messages, and component stack traces inside an `{import.meta.env.DEV && ...}` check. This ensures a clean, non-leaking user experience in production while retaining high developer velocity during local debug sessions.
