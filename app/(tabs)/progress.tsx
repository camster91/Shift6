...
import type { Program, ProgramVersion, TrainingCycle } from '../../src/domain/types';
...
export default function ProgressScreen() {
    ...
    void (async () => {
      try {
        ...
        const selectedExerciseProgress = nextSelectedExerciseId
          ...
          : null;
        const nextSessionTargets = workout
  ...
  if (loading || (database && currentCycle && loadedFor?.database !== database)) {
    return (
      <Screen>
  ...
  if (!database || !currentCycle || !currentProgram || !summary || loadedFor?.userId !== userId) {
    return (
      <Screen>
        <ErrorState
          ...
          message={
            error ??
            'No saved cycle and program are available on this device. Start or recover a cycle from Home.'
          }
        />
