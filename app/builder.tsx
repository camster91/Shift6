...
const starterPlank = foundationalExercises.find((exercise) => exercise.id === 'exercise-plank');
const demoCatalogueEntry = programLibrary.find((entry) => entry.program.id === demoProgram.id);

type BuilderContext = {
  userId: string;
  mode: string | undefined;
  sourceVersionId: string | undefined;
  database: ReturnType<typeof useLocalDatabase>;
};

function sameBuilderContext(left: BuilderContext, right: BuilderContext): boolean {
  return (
...
export default function ProgramBuilderScreen() {
  ...
  const currentContext: BuilderContext = { userId, mode, sourceVersionId, database };
  const currentContextRef = useRef(currentContext);
  currentContextRef.current = currentContext;
  const requiresSavedSource = Boolean(sourceVersionId && !isBlankBuilder);
  const isDefaultDemoCopy = !isBlankBuilder && !sourceVersionId;
  const defaultDemoCopyAllowed =
    !isDefaultDemoCopy ||
    (demoCatalogueEntry !== undefined && isProgramStartAllowed(demoCatalogueEntry, __DEV__));
  const builderTitle =
    mode === 'progress'
  ...
  const [draftContext, setDraftContext] = useState(currentContext);
  const [loadedSourceVersionId, setLoadedSourceVersionId] = useState<string | null>(null);
  ...
  const draftMatchesContext = sameBuilderContext(draftContext, currentContext);
  const savedSourceReady =
    !requiresSavedSource ||
    (Boolean(database) &&
      !sourceLoading &&
      !sourceLoadError &&
      loadedSourceVersionId === sourceVersionId);
  const canMutateDraft = () =>
    sameBuilderContext(currentContextRef.current, currentContext) &&
    draftMatchesContext &&
    savedSourceReady;

  useEffect(() => {
    if (sameBuilderContext(draftContext, currentContext)) return;
    setDraft(createInitialBuilderDraft(isBlankBuilder, userId));
    setDraftContext(currentContext);
    setLoadedSourceVersionId(null);
    setAvailableEquipmentIds(demoUser.equipmentIds);
    setUnitSystem(demoUser.unitSystem);
    setCustomExercises({});
    setCustomName('');
    setCustomInstructions('');
    setCustomNotes('');
    setNewWorkoutTitle('');
    setNewWorkoutSequence(0);
    ...
    setSourceLoading(shouldLoadSource);
    setSourceLoadError(false);
    setSaved(false);
    setSaving(false);
    setStarting(false);
    setError(null);
    trackedProgramCreation.current = false;
  }, [database, mode, sourceVersionId, userId]);
  ...
  useEffect(() => {
    ...
    void Promise.all([getOnboardingProfile(database, userId), getUserExercises(database, userId)])
      .then(([profile, userExercises]) => {
        if (!active || !sameBuilderContext(currentContextRef.current, currentContext)) return;
        if (profile) {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      .then(async ([snapshot, userExercises, latestCycle, profile]) => {
        ...
        setDraft(() => nextDraft);
        setLoadedSourceVersionId(sourceVersionId);
      ...
      .catch(() => {
        if (active && sameBuilderContext(currentContextRef.current, currentContext)) {
          setSourceLoadError(true);
  ...
  const handleAddPlank = (workoutId: string | undefined = firstWorkout?.id) => {
    if (!workoutId) return;
    if (!starterPlank || !isExerciseAvailableToUser(starterPlank, __DEV__)) {
      setError(
        'This starter movement is still under content review. Choose an available movement.',
      );
      return;
    }
    setDraft((current) => ({
  ...
  const handleAddCatalogueExercise = (workoutId: string, exercise: Exercise) => {
    if (!isExerciseAvailableToUser(exercise, __DEV__)) {
      setError('This movement is still under content review. Choose an available movement.');
      return;
    }
  ...
  const handleSave = async () => {
    if (saving) return;
    if (!canMutateDraft()) {
      setError('Wait for your private draft to finish loading before saving.');
      return;
    }
    if (!defaultDemoCopyAllowed) {
      setError('This source program is still under content review. Start from blank instead.');
      return;
    }
    if (!database && !__DEV__) {
      setError('Local training storage is unavailable. Your plan has not been saved.');
      return;
    }
    ...
    try {
      ...
      if (canMutateDraft()) {
        setError(
          saveError instanceof Error ? saveError.message : 'We could not save this program.',
        );
      }
    } finally {
      if (sameBuilderContext(currentContextRef.current, currentContext)) setSaving(false);
    }
  };
  ...
  const handleStartCycle = async () => {
    ...
    if (!canMutateDraft()) {
      setError('Wait for your private draft to finish loading before starting.');
      return;
    }
    if (!defaultDemoCopyAllowed) {
      setError('This source program is still under content review. Start from blank instead.');
    ...
    if (!database && !__DEV__) {
      setError('Local training storage is unavailable. Your plan has not been started.');
      return;
    }
    ...
    try {
      ...
      if (canMutateDraft()) {
        setError(
          startError instanceof Error ? startError.message : 'We could not start this cycle.',
        );
      }
    } finally {
      if (sameBuilderContext(currentContextRef.current, currentContext)) setStarting(false);
    }
  };
  ...
  if (
    !draftMatchesContext ||
    sourceLoading ||
    (shouldLoadSource && !savedSourceReady && !sourceLoadError)
  ) {
    return (
      <Screen>
  ...
  if (requiresSavedSource && !database) {
    return (
      <Screen>
        <ErrorState message="A saved private program needs local storage before it can be edited." />
      </Screen>
    );
  }
    ...
    return (
      <Screen>
        <IconButton
          ...
          onPress={() => router.back()}
        />
        <ErrorState message="This source program is under content review and is not available to copy yet." />
        <Button label="Back to Programs" onPress={() => router.replace('/(tabs)/programs')} />
      </Screen>
    );
  }
  ...
  return (
    <Screen scrollViewProps={{ keyboardShouldPersistTaps: 'handled' }}>
        ...
        <Card key={workout.id} tone="white" style={styles.workoutCard}>
          ...
          {workout.exercises.length === 0 ? (
            <View style={styles.emptyWorkout}>
              ...
              {starterPlank && isExerciseAvailableToUser(starterPlank, __DEV__) ? (
                <Button
      ...
      {!isBlankBuilder && starterPlank && isExerciseAvailableToUser(starterPlank, __DEV__) ? (
        <Button
      ...
      <Button
        ...
        onPress={handleSave}
        disabled={!defaultDemoCopyAllowed}
        loading={saving}
      ...
      <Text variant="caption" tone="muted" style={styles.startNote}>
        {defaultDemoCopyAllowed
          ? 'Starting snapshots this private version. The public template and completed history stay unchanged.'
          : 'This source program is under content review. Build from blank to create a private plan.'}
      </Text>
    </Screen>
  );
}
