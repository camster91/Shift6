...
export default function WorkoutSummaryScreen() {
    ...
    void (async () => {
      try {
        ...
        const snapshot = localSession
          ...
          : null;
        const localWorkout = snapshot?.version.workouts.find(
        ...
        setSession(
          localWorkout &&
            (localSession?.status === 'complete' ||
              localSession?.status === 'partial' ||
              localSession?.status === 'skipped')
            ? localSession
            : null,
        );
        setSets(localSets);
        setExistingCheckIn(checkIn);
        setWorkout(localWorkout ?? null);
        setLoadedFor({ database, userId, sessionId });
        if (checkIn) {
  ...
  const facts = useMemo(
    () => (session && workout ? buildSummaryFacts(session, sets, workout) : null),
    [session, sets, workout],
  );
  const isPartial = session?.status === 'partial';
  const isSkipped = session?.status === 'skipped';
  const isEarlyStop = isPartial || isSkipped;
  const displayedSetCount = facts?.setCount ?? 0;
  ...
  const saveCheckIn = async () => {
    ...
    try {
      await saveWorkoutCheckIn(database, checkIn);
      setExistingCheckIn(checkIn);
      setSaved(true);
    } catch (saveError) {
  ...
  if (loading || (database && session && loadedFor?.database !== database)) {
    return (
      <Screen>
  ...
  if (
    ...
    return (
      <Screen>
        <ErrorState
          title="Workout summary unavailable"
          message={
            error ??
            'This completed session and its saved workout are not available on this device.'
          }
        />
    ...
    <Screen>
      ...
      >
        ...
        <View style={styles.factsRow}>
          ...
          <SummaryFact
            label="Status"
            value={isPartial ? 'Saved partial' : isSkipped ? 'Saved skipped' : 'Saved'}
          />
        </View>
      </Card>
      ...
      <Text variant="caption" tone="muted" style={styles.persistenceNote}>
        Your check-in is saved on this device and queued for sync when an account backend is
        connected.
      </Text>
    </Screen>
  );
