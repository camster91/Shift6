...
**Run date:** 2026-06-06
**Round:** 2 of 3
**Output:** `~/Shift6/docs/tester-reports/tester-6-alex-round2.md`
...
Write to `~/Shift6/docs/tester-reports/tester-6-alex-round2.md`:

```
# Athlete Alex — Round 2
...
## Round 2 — Edge cases (Alex, ~30 min)

You are now a returning user. You have an account, real 1RMs.

1. **Sign up fresh.** New email alias.
2. **Forgot password.**
3. **Set 1RMs accurately.** Squat 405, Bench 285, Deadlift 495. Verify the math downstream.
4. **Walk through all 6 weeks of periodization.** Can you skip to week 6 (Deload)? Does it actually deload? Is it 58-62% of 1RM with reduced volume?
5. **RPE capture.** Log a set with RPE 9. Does the next set's weight adjust? (A real periodization app will drop the weight if RPE is high.)
6. **PR detection accuracy.** Log a set at 95% of 1RM. Does the app celebrate a PR? (It shouldn't — that's not a PR, that's a working set.) Then log a set at 102% of 1RM. Does THAT get a PR celebration?
7. **Plate visualizer.** 405 lbs on the bar. Bar is 45. Plates per side: (405-45)/2 = 180 lbs. 45+45+45+45 = 180. Four 45-lb plates per side. Does the visualizer show four 45s?
8. **Modifier math.** Toggle High CNS Fatigue. The primary compound should drop to 60% of 1RM. Does the math reflect that?
9. **Data export for coach.** Is there a CSV export? Does it include RPE per set? Notes per set? Equipment used?
10. **Background the app mid-workout.** Rest timer survives?
11. **Delete account.** Where?

For each step:
- Was the math right?
- Did PR detection fire when it should and not when it shouldn't?
- Did the deload actually deload?
- Anything labeled "periodization" that didn't actually do periodization math


## After testing

Write the report to `~/Shift6/docs/tester-reports/tester-6-alex-round2.md`.

Then output a one-line summary in your final response:

```
WROTE: ~/Shift6/docs/tester-reports/tester-6-alex-round2.md — <one-line summary of top finding>
```
