...
export default function PlateVisualizer({ weight, track = 'full_gym', compact = false }) {
  ...
  if (weight < BARBELL_BAR_LBS) {
    return (
      <div className={`flex ${compact ? 'flex-row items-center gap-3' : 'flex-col items-center gap-2'} mt-1`}>
        ...
        <BarbellSvg weight={BARBELL_BAR_LBS} plates={[]} barOnly compact={compact} />
  ...
  return (
    <div className={`flex ${compact ? 'flex-row items-center gap-2' : 'flex-col items-center gap-2'} mt-1`}>
      ...
      <BarbellSvg weight={weight} plates={plates} compact={compact} />
...
function BarbellSvg({ weight, plates = [], barOnly = false, compact = false }) {
  const label = barOnly
    ? `${BARBELL_BAR_LBS} pound barbell, bar only`
    : `Barbell loaded to ${weight} pounds. ${plates.length > 0 ? 'Plates per side: ' + plates.map(p => p.weight).join(', ') : 'Empty bar'}.`;
...
function DumbbellSvg({ size, color, weight, compact = false }) {
  const label = `${weight} pound dumbbell`;
