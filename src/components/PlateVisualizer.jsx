...
function BarbellSvg({ plates = [], barOnly = false, compact = false, weight }) {
  ...
  return (
    <svg
      ...
      role="img"
      aria-label={`Barbell visualization for ${weight} lbs. ${barOnly ? '45 lb bar only.' : `45 lb bar with ${plates.length} plates per side.`}`}
    >
...
function DumbbellSvg({ size, color, weight, compact = false }) {
  ...
  return (
    <svg
      ...
      className="drop-shadow-md"
      role="img"
      aria-label={`Dumbbell visualization for ${weight} lbs.`}
    >
