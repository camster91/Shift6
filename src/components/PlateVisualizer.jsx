...
function BarbellSvg({ plates = [], barOnly = false, compact = false, weight }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width={W} height={H} className="drop-shadow-md" role="img" aria-label={`Barbell loaded to ${weight} lbs`}>
...
function DumbbellSvg({ size, color, weight, compact = false }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width={W} height={H} className="drop-shadow-md" role="img" aria-label={`${weight} lb dumbbell`}>
