...
const DayDetailModal = ({ date, workouts, plannedExercises, isPast, isToday, isFuture, onClose, onStartWorkout, allExercises, theme = 'dark' }) => {
    if (!date) return null
    ...
    return (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4" onClick={onClose}>
            <div
                className={`${tc.cardBg} border ${tc.border} rounded-xl w-full max-w-md max-h-[85vh] overflow-hidden flex flex-col`}
                onClick={e => e.stopPropagation()}
            >
                <div className={`flex items-center justify-between p-4 border-b ${tc.border}`}>
                    <div>
                        <h3 className={`text-lg font-bold ${tc.textPrimary}`}>{dateStr}</h3>
                        {!isRestDay && (
                            <p className="text-xs text-cyan-400">Workout Day</p>
                        )}
                    </div>
                    <button onClick={onClose} className={`p-2 ${tc.hoverBg} rounded-lg transition-colors`}>
                ...
                <div className="p-4 overflow-y-auto flex-1">
                    {isRestDay ? (
                        <div className="text-center py-8">
                            ...
                            <p className={`${tc.textMuted} text-sm mt-1`}>Recovery is part of the program</p>
                        </div>
                    ) : (
                        <div className="space-y-4">
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <div className="flex-1">
                                                    <p className={`font-semibold ${tc.textPrimary} text-sm`}>
                            ...
                            {plannedExercises.length > 0 && (
                                <div className="space-y-2">
                                    <p className={`text-xs ${tc.textMuted} uppercase tracking-wider flex items-center gap-2`}>
                                        <Clock size={12} />
                                    ...
                                    {plannedExercises.map((planned, i) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <div className="flex-1">
                                                    <p className={`font-semibold text-sm ${isPast ? tc.textMuted : tc.textPrimary}`}>
                                                ...
                                                {(isToday || isFuture) && onStartWorkout && (
                                                    <button
                                                        ...
                                                        className={`px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1 transition-colors ${colorClass} text-white active:opacity-90`}
                                                    >
                            ...
                            {workouts.length === 0 && plannedExercises.length === 0 && (
                                <div className="text-center py-8">
                                    <div className={`w-12 h-12 ${theme === 'light' ? 'bg-slate-200' : 'bg-slate-800'} rounded-full flex items-center justify-center mx-auto mb-3`}>
                                        <Dumbbell size={20} className={tc.textMuted} />
                                    </div>
                                    <p className={`${tc.textMuted} text-sm`}>No exercises scheduled</p>
                                </div>
                            )}
                        </div>
                    )}
...
const CalendarView = ({ sessionHistory, completedDays = {}, allExercises = {}, activeProgram = [], startWorkout, theme = 'dark' }) => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        const isDayToday = isCurrentMonth && today.getDate() === day
        ...
        calendarDays.push(
            <button
                ...
                className={`h-10 md:h-12 flex flex-col items-center justify-center rounded-lg relative transition-all cursor-pointer ${
                    isDayToday
                        ? 'bg-cyan-500/20 border border-cyan-500/50 active:bg-cyan-500/30'
                        : hasWorkout
                            ? theme === 'light'
                                ? 'bg-slate-200/50 active:bg-slate-200'
            ...
            >
                <span className={`text-sm font-medium ${isDayToday ? 'text-cyan-400' : tc.textSecondary}`}>
                    {day}
                </span>
                {hasWorkout && (
                    <div className="flex gap-0.5 mt-0.5">
                        ...
                        {exerciseColors.length > 3 && (
                            <span className={`text-[8px] ${tc.textMuted}`}>+{exerciseColors.length - 3}</span>
                        )}
                    </div>
                )}
            </button>
    ...
    return (
        <>
            <div className={`${tc.cardBg} border ${tc.border} rounded-xl overflow-hidden hover:border-cyan-500/30 transition-colors`}>
                {/* Header */}
                <div className={`p-4 border-b ${tc.border} flex items-center justify-between`}>
                    ...
                    >
                        <ChevronLeft size={18} className={tc.textMuted} />
                    </button>
                    <div className="text-center">
                        <h3 className={`text-sm font-semibold ${tc.textPrimary}`}>{MONTHS[month]} {year}</h3>
                        <p className={`text-xs ${tc.textMuted}`}>{totalWorkoutsThisMonth} workouts • {daysWithWorkouts} days</p>
                    </div>
                    <button
                        onClick={nextMonth}
                        className={`p-2 ${tc.hoverBg} rounded-lg transition-colors`}
                    ...
                    >
                        <ChevronRight size={18} className={isCurrentMonth ? (theme === 'light' ? 'text-slate-300' : 'text-slate-700') : tc.textMuted} />
                    </button>
                </div>

                {/* Days of week header */}
                <div className={`grid grid-cols-7 gap-1 px-3 py-2 border-b ${theme === 'light' ? 'border-slate-200/50' : 'border-slate-800/50'}`}>
                    {DAYS_OF_WEEK.map(day => (
                        <div key={day} className={`text-center text-[10px] font-bold ${tc.textMuted} uppercase`}>
                            {day}
                        </div>
                    ))}
                </div>
                ...
                <div className="px-4 pb-3">
                    <p className={`text-[10px] ${tc.textMuted} text-center`}>Tap any day to view workout details</p>
                </div>
            </div>
