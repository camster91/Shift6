                        ...
                        <div className="flex items-center gap-2 text-blue-600 font-black uppercase tracking-[0.3em] text-[10px] relative">
                            ...
                            {activeTooltip === 'system' && (
                                <div className="absolute top-6 left-0 z-50 bg-slate-900 text-white text-[9px] p-3 rounded-xl w-48 border border-white/10 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
                                    Adaptive engine is live. Monitoring failure points and volume accumulation across all active nodes.
                                </div>
                            )}
                        </div>
                        <h1 className="text-5xl md:text-7xl font-black tracking-tighter text-slate-900 leading-none uppercase">Command Center</h1>
                        <p className="text-slate-400 font-medium text-lg">Your elite baseline is recalibrating.</p>
                    </div>
                </div>

                {/* Hero Section - Full Width */}
                <div className="bg-slate-900 text-white p-10 md:p-16 rounded-[4rem] relative overflow-hidden shadow-[0_40px_80px_-15px_rgba(15,23,42,0.3)] ring-1 ring-white/10 mesh-bg selection:bg-blue-500/30">
                    ...
                    <div className="relative z-10 flex flex-col lg:flex-row gap-16 items-center">
                        <div className="flex-1 space-y-12 w-full">
                            <div>
                                <div className="flex items-center gap-2 mb-4 relative">
                                    <p className="text-blue-400 font-black uppercase tracking-[0.4em] text-[11px]">Volume Overload Trajectory</p>
                                    <button
                                    ...
                                    {activeTooltip === 'volume' && (
                                        <div className="absolute top-6 left-0 z-50 bg-slate-800 text-white text-[10px] p-4 rounded-2xl w-64 border border-white/10 shadow-2xl animate-in fade-in zoom-in-95 duration-200 font-medium normal-case tracking-normal">
                                            Total volume (reps × sets) over the last 32 days. Spikes represent sessions pushed to physiological failure.
                                        </div>
                                    )}
                                </div>
                                <h2 className="text-6xl md:text-8xl font-black mb-6 tracking-tighter leading-tight italic">Elite Potential</h2>
                                <p className="text-slate-400 text-lg font-medium leading-relaxed max-w-xl">
                                    Compounding execution across 18 weeks. Your current volume is shifting <span className="text-white">12% above</span> the projected baseline. Stay calibrated.
                                </p>
                            </div>
                                ...
                                {Array.from({ length: 32 }).map((_, i) => {
                                    ...
                                    return (
                                        <div key={i} className="flex-1 bg-white/[0.03] rounded-t-sm relative group h-full">
                                            <div
                                                className={`absolute bottom-0 left-0 right-0 bg-blue-500 rounded-t-sm transition-all duration-[1500ms]`}
                                                style={{
                        ...
                        <div className="w-full lg:w-96 bg-white/5 backdrop-blur-2xl rounded-[3.5rem] p-12 border border-white/10 flex flex-col justify-between shadow-3xl">
                            <div className="space-y-4">
                                <p className="text-[12px] font-black uppercase tracking-[0.3em] text-slate-500">Active Stack</p>
                                <h3 className="text-5xl font-black tracking-tighter">
                                    {workoutQueue.length > 0 ? `${workoutQueue.length + 1} S` : (dailyStack.length > 0 ? `${dailyStack.length} X` : 'CLEAN')}
                                </h3>
                            ...
                            <div className="space-y-6 pt-12">
                                {workoutQueue.length > 0 ? (
                                    <button
                                        ...
                                        className="w-full bg-rose-600/20 hover:bg-rose-600/30 text-rose-400 py-7 rounded-[2.5rem] font-black text-xs uppercase tracking-[0.3em] transition-all border border-rose-500/20 active-scale"
                                    >
                                        Exit Stack
                                    </button>
                                ) : (
                                    <button
                                        ...
                                        className="w-full bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white py-7 rounded-[2.5rem] font-black text-xs uppercase tracking-[0.3em] transition-all shadow-[0_20px_50px_-10px_rgba(37,99,235,0.5)] active-scale group flex items-center justify-center gap-4"
                                    >
                                        Launch Protocol <Zap size={16} className="group-hover:animate-pulse fill-white" />
                                    </button>
                                )}
                            </div>
                        </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                ...
                <div className="lg:col-span-4 space-y-12">
                    ...
                    <div className="bg-slate-900 rounded-[3.5rem] p-12 text-white shadow-3xl active-scale group relative overflow-hidden mesh-bg ring-1 ring-white/5">
                        <div className="absolute -bottom-10 -right-10 opacity-10 group-hover:scale-150 transition-transform duration-[2000ms] pointer-events-none">
                            <Volume2 size={300} />
                        </div>
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="mb-12 relative z-10">
                                        <div className={`w-20 h-20 rounded-[2rem] flex items-center justify-center mb-8 transition-all group-hover:scale-110 group-hover:-rotate-6 duration-500 shadow-2xl bg-${ex.color}-600 text-white shadow-${ex.color}-900/20`}>
                                            {React.cloneElement(ex.icon, { size: 36, strokeWidth: 2.5 })}
                                        </div>
                                    ...
                                    <div className="w-full bg-slate-50 h-3.5 rounded-full overflow-hidden p-1 border border-slate-100">
                                        <div
                                            className={`h-full rounded-full transition-all duration-[1500ms] ease-out shadow-sm bg-${ex.color}-600`}
                                            style={{ width: `${percent}%` }}
                                        />
                                    </div>
                                </button>
