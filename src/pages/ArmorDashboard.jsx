...
const HabitCheck = React.memo(({ habit, done, toggleHabit }) => {
  return (
    <button
...
export default function ArmorDashboard({ onStartWorkout, onNavigateToSettings }) {
  ...
  const { top1RM, allTrack1RMsZero, displayTop1RM } = useMemo(() => {
    ...
    const top = track1RMs.length > 0 ? Math.max(...track1RMs, 0) : 0;
  ...
  // PERFORMANCE: Stabilize icons and labels passed to StatTile (React.memo)
  const weekIcon = useMemo(() => <span aria-hidden="true">📅</span>, []);
  const phaseIcon = useMemo(() => <span aria-hidden="true">⚡</span>, []);
  const rmIcon = useMemo(() => <span aria-hidden="true">🏆</span>, []);
  const rmLabel = useMemo(() => (
    ...
    <div className="pb-32 space-y-5 max-w-lg mx-auto">
      ...
      <div className="px-5 space-y-5">
        ...
        <div className="grid grid-cols-3 gap-2">
          <StatTile icon={weekIcon} label="Week" value={`${currentCycle.week}/6`} accent="cyan" />
          <StatTile icon={phaseIcon} label="Phase" value={weekConfig.phase} accent="emerald" />
          <StatTile
            icon={rmIcon}
            label={rmLabel}
            value={displayTop1RM}
