...
export default function Shift6Dashboard({ onStartWorkout, onNavigateToSettings }) {
  ...
  // PERFORMANCE: Memoize 1RM derived values to prevent redundant computation
  // on every re-render of the dashboard.
  const { top1RM, allTrack1RMsZero } = useMemo(() => {
    ...
    const zero = rms.length > 0 && rms.every(v => v === 0);
    return { top1RM: top, allTrack1RMsZero: zero };
  }, [effectiveTrack, estimated1RMs]);

  // PERFORMANCE: Memoize workout history derivations for WeekStrip
  const completedDates = useMemo(() =>
    workoutHistory.filter(w => w.completed).map(w => w.date),
    [workoutHistory]
  );
  const mvdDates = useMemo(() =>
    streakData.mvdDates || [],
    [streakData.mvdDates]
  );
