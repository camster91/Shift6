...
// PERFORMANCE: Memoized HabitPill component to prevent redundant button renders during
// parent context updates. Every millisecond counts.
...
export default function Shift6Dashboard({ onStartWorkout, onNavigateToSettings }) {
  ...
  // PERFORMANCE: Memoize completedDates O(N) filter and map operation to avoid redundant array allocations on every render.
  const completedDates = useMemo(() =>
    workoutHistory.filter(w => w.completed).map(w => w.date),
    [workoutHistory]
  );

  // PERFORMANCE: Memoize 1RM track stats O(E) derivation to avoid recalculating on every render, and prune unused stats.
