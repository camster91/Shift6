...
class WorkoutErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, info) {
    // Technical details are logged to console for debugging but suppressed in UI
    console.error('[WorkoutErrorBoundary CAUGHT]', error?.message || String(error), error?.stack?.slice(0, 800), info?.componentStack?.slice(0, 500));
  }
  render() {
    if (this.state.hasError) {
      return (
        <div className="flex flex-col flex-1 px-6 pt-4 text-center max-w-sm mx-auto w-full items-center justify-center space-y-4 p-4">
          ...
          <p className="text-[var(--color-cardio)] text-[10px] font-medium italic">A technical error occurred. Details have been logged to the console.</p>
